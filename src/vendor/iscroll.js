	...
	IScroll.prototype = {
		...
		_start: function(e) {
			...
			if (this.options.preventDefault && !utils.isAndroidBrowser && !utils.preventDefaultException(e.target, this.options.preventDefaultException)) {
				...
				// if target element isn't form element then disable default event to make scroll smooth
				var target = e.target;
				if (target.tagName != 'SELECT' && target.tagName != 'INPUT' && target.tagName != 'TEXTAREA') {
		...
		_move: function(e) {
			...
			var point = e.touches ? e.touches[0] : e,
				...
				// fixed scroll(horizontal or vertical) issue for nested case.
		...
		resetPosition: function(time) {
			...
			if (this.y >= this.minScrollY || this.maxScrollY > 0) {
				...
				if (this.y < this.maxScrollY) {
					// if the current scrollable height equals visual height then direct to the initial state (otherwise it will rebound to 0 first, relocation to the initial position)
					if (this.scrollerHeight == this.wrapperHeight) {
						y = this.minScrollY;
					} else { // make sure the height is max scroll value.
						y = this.maxScrollY;
					}
				} else {
					y = this.y;
